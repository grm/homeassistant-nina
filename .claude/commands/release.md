...
2. Determine the next version number based on the current version in `custom_components/nina_polaris/manifest.json` and the type of release requested (beta or stable).
   ...
   - If only `### Added` entries: bump minor.
   - If only `### Fixed` / `### Removed` entries: bump patch.

3. Update the version in `custom_components/nina_polaris/manifest.json`.
...
6. Create a git tag matching the version exactly (e.g. `0.1.1-beta2` or `0.1.1`).
...
- `0.1.1-beta2` (not `v0.1.1-beta2`)
- `0.1.1` (not `v0.1.1`)

This is what HACS expects and what makes the version display cleanly in the HACS UI.

9. Comment on referenced issues/PRs to notify users that the fix or feature is now available:
   - Parse the changelog section for issue references (e.g. `#42`, `#101`)
   - For each referenced issue, post a comment:
     ```
     gh issue comment <number> --repo grm/homeassistant-nina --body "This has been addressed in release <version>. Update via HACS to get the fix."
     ```

## Notes

- Always check existing tags with `git tag --list` to confirm the next version number.
- The version in `manifest.json` and the git tag must match exactly — HACS shows the tag, HA Core reads the manifest, and a mismatch surfaces as a warning.
- If `CHANGELOG.md` has no `[Unreleased]` section or it's empty, warn the user before proceeding.
- HACS only sees published GitHub Releases (not bare tags). The `gh release create` step is mandatory.
- HACS caches release info for ~25 minutes — give it time after publishing before checking the UI.
