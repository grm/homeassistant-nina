...
- **Dashboard visual overhaul (Option 1 — tile grid)** — *in progress, refactor by section*. The Camera section now renders as a 2-column grid of native HA `tile` cards (Sensor temp / Cooler power / Cooler / Exposing) with adapted MDI icons (`mdi:thermometer`, `mdi:snowflake`, `mdi:snowflake-thermometer`, `mdi:camera-iris`) and color coding instead of a flat `entities` list. The redundant `Trevinca …` prefix is dropped from labels (the dashboard title already names the instance). Mount, Guiding, Weather, Focuser, and Sequence sections will follow in subsequent commits.
